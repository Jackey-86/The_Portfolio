...
const Lightbox: React.FC<{ images: string[]; startIdx: number; name: string; onClose: () => void }> = ({ images, startIdx, name, onClose }) => {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.96)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div onClick={e => e.stopPropagation()} style={{ position: 'relative', width: '100%', maxWidth: 1100, maxHeight: '90vh', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: -36, right: 0, background: 'none', border: 'none', color: 'rgba(255,255,255,0.6)', fontSize: '1rem', cursor: 'pointer', fontFamily: mono }}>✕ close</button>
        <img src={images[idx]} alt={`${name} ${idx + 1}`}
          style={{ maxWidth: '100%', maxHeight: '78vh', objectFit: 'contain', borderRadius: 4, display: 'block', userSelect: 'none' }} />
          ...
          <>
            {[{ label: '‹', fn: prev, side: 'left' }, { label: '›', fn: next, side: 'right' }].map(btn => (
              <button key={btn.side} onClick={btn.fn}
                style={{ position: 'absolute', top: '45%', [btn.side]: -8, transform: 'translateY(-50%)', width: 38, height: 38, borderRadius: '50%', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.15)', color: '#fff', fontSize: '1.2rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                onMouseEnter={e => (e.currentTarget as HTMLButtonElement).style.background = 'rgba(0,255,136,0.3)'}
                onMouseLeave={e => (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.08)'}>
                {btn.label}
              </button>
            ))}
            <div style={{ display: 'flex', gap: 6, marginTop: '0.75rem', flexWrap: 'wrap', justifyContent: 'center' }}>
              {images.map((_, i) => (
...
const ProjectModal: React.FC<{ project: PortfolioProject; onClose: () => void; onStartProject: () => void }> = ({ project: p, onClose, onStartProject }) => {
  ...
  return (
    <>
      ...
      <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 1500, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '1rem', overflowY: 'auto' }}>
        <div onClick={e => e.stopPropagation()}
          style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 8, width: '100%', maxWidth: 820, margin: 'auto', animation: 'fadeUp 0.25s ease' }}>

          {/* Header */}
          <div style={{ padding: '1.25rem', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
            <div style={{ minWidth: 0 }}>
              ...
              <h2 style={{ fontSize: 'clamp(1.1rem, 4vw, 1.5rem)', fontWeight: 800, marginBottom: '0.25rem' }}>{p.name}</h2>
              <div style={{ fontFamily: mono, fontSize: '0.68rem', color: 'var(--cyan)' }}>{p.category}{p.year ? ` · ${p.year}` : ''}</div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
              <span style={{ fontFamily: mono, fontSize: '0.6rem', color: 'var(--green)', background: 'rgba(0,255,136,0.08)', border: '1px solid rgba(0,255,136,0.2)', padding: '0.2rem 0.55rem', borderRadius: 3 }}>{p.status}</span>
          ...
          {p.images?.length > 0 && (
            <div style={{ padding: '1.25rem', borderBottom: '1px solid var(--border)' }}>
              <div style={{ fontFamily: mono, fontSize: '0.58rem', color: 'var(--fg3)', letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>images — tap to enlarge</div>
              <div style={{ display: 'grid', gridTemplateColumns: p.images.length === 1 ? '1fr' : 'repeat(auto-fill, minmax(140px, 1fr))', gap: '0.5rem' }}>
                {p.images.map((url, i) => (
                  <div key={i} onClick={() => setLightboxIdx(i)}
                    style={{ aspectRatio: '16/9', overflow: 'hidden', borderRadius: 4, border: '1px solid var(--border)', cursor: 'zoom-in' }}
          ...
          <div style={{ padding: '1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              {p.project_url && (
                <a href={p.project_url} target="_blank" rel="noreferrer" className="btn-secondary" style={{ fontSize: '0.7rem', padding: '0.45rem 1rem' }}>view_live →</a>
              )}
...
const Carousel: React.FC<{ images: string[]; name: string; onImageClick: (i: number) => void }> = ({ images, name, onImageClick }) => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', aspectRatio: '16/9', overflow: 'hidden', background: '#000', borderRadius: '6px 6px 0 0' }}>
      <img src={images[idx]} alt={`${name} ${idx + 1}`} onClick={() => onImageClick(idx)}
      ...
      {images.length > 1 && (
        <>
          {[{ label: '‹', fn: () => go((idx - 1 + images.length) % images.length), side: 'left' },
            { label: '›', fn: () => go((idx + 1) % images.length), side: 'right' }].map(btn => (
            <button key={btn.side} onClick={e => { e.stopPropagation(); btn.fn(); }}
              style={{ position: 'absolute', top: '50%', [btn.side]: 6, transform: 'translateY(-50%)', width: 30, height: 30, borderRadius: '50%', background: 'rgba(0,0,0,0.55)', border: '1px solid rgba(255,255,255,0.18)', color: '#fff', fontSize: '1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2 }}>
              {btn.label}
            </button>
          ))}
          <div style={{ position: 'absolute', bottom: 8, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 4, zIndex: 2 }}>
            {images.map((_, i) => (
              <button key={i} onClick={e => { e.stopPropagation(); go(i); }}
                style={{ width: i === idx ? 16 : 6, height: 6, borderRadius: 3, background: i === idx ? 'var(--green)' : 'rgba(255,255,255,0.4)', border: 'none', cursor: 'pointer', padding: 0, transition: 'all 0.3s' }} />
            ))}
          </div>
          <div style={{ position: 'absolute', top: 8, right: 8, fontFamily: mono, fontSize: '0.58rem', color: 'rgba(255,255,255,0.7)', background: 'rgba(0,0,0,0.4)', padding: '0.18rem 0.45rem', borderRadius: 3 }}>
            {idx + 1}/{images.length}
          </div>
        </>
      )}
...
const Works: React.FC<WorksProps> = ({ onStartProject }) => {
  ...
  return (
    <>
      <style>{`
        .works-section { padding: 4rem 1.5rem; max-width: 1100px; margin: 0 auto; }
        .works-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 1.5rem; }
        @media (max-width: 640px) {
          .works-section { padding: 3rem 1rem; }
          ...
          <div className="works-grid">
            {filtered.map((p, i) => (
              <div key={p.id}
                ...
                <Carousel images={p.images || []} name={p.name} onImageClick={idx => setLightboxState({ project: p, idx })} />
                ...
                <div style={{ padding: '1.25rem' }} onClick={() => setSelected(p)}>
                  ...
                  <div style={{ marginTop: '1rem', fontFamily: mono, fontSize: '0.62rem', color: 'var(--green)' }}>view details →</div>
                </div>
              </div>
            ))}
          </div>
